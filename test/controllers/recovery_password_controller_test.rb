...
class RecoveryPasswordControllerTest < ActionController::TestCase
  ...
  test "should exist a field with name 'recovery_password[email_or_username]' in the form with name 'recovery_password_form'" do
    get :index
    assert_select 'form[name="recovery_password_form"] input[name="recovery_password[email_or_username]"]', 1, 'Input with name "email_or_username" not found.'
  end
